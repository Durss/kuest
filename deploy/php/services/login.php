<?php
	...
	&& (!isset($_SESSION['kuest_logged']) || $_SESSION['kuest_logged'] === false)
	...
	//if (isset($_POST["pubkey"], $_POST["uid"], $_POST["samples"])) {
		...
		$params = array(':uid' => $_SESSION['kuest_uid'], ':uid2' => "%,".$_SESSION['kuest_uid'].",%");
		$req = DBConnection::getLink()->prepare($sql);
		...
		//*
		foreach ($_SESSION['kuest_friends'] as $row) {
