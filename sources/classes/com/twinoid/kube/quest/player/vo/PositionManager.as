package com.twinoid.kube.quest.player.vo {
	...
	public class PositionManager {
		 ...
		 */
		public function populate(nodes:Vector.<KuestEvent>, debugMode:Boolean):void {
			_posToEvents = {};
			var i:int, len:int;
			len = nodes.length;
			for (i = 0; i < len; ++i) {
				var id:String = nodes[i].actionPlace == null && debugMode? getPositionId(new Point()) : getPositionId(nodes[i].actionPlace);
		...
		public function getEventsFromPos(pos:*):Vector.<KuestEvent> {
			if(_posToEvents == null) return new Vector.<KuestEvent>();
