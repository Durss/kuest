package com.twinoid.kube.quest.editor.cmd {
	 ...
	 * @date 28 avr. 2013;
	 */
	public class SaveQuestCmd extends EventDispatcher implements ProgressiveCommand {
...
//			_loader.addEventListener(ProgressEvent.PROGRESS, dispatchEvent);//Can't capture upload progress... :(. Only work with FileReference.upload()
		...
		public function populate(title:String, description:String, data:ByteArray, friends:Array, callback:Function, guid:String = "", publish:Boolean = false):void {
			...
			if(StringUtils.trim(guid).length > 0) url = url+"&id="+guid;
